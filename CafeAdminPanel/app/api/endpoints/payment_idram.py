...
def start_payment(payment_data: PaymentIDramInitiationSchema,
                    payment_service: idram_payment_service.IDramPaymentService = Depends(),
                    current_admin=Depends(cafe_admin_auth_service.get_current_admin)
                    ):

    user_id = current_admin.__dict__.get("id")

    return payment_service.start_payment(user_id, payment_data)
